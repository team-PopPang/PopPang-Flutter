...
### 2. v1에서는 Flutter repository가 관리자 API를 직접 호출해요

v1에서는 Flutter feature가 자기 repository 구현체를 통해 관리자 API를 직접 호출해요.

대신 아래 전제를 지켜야 해요.

- Flutter `usecase`는 자기 feature의 `repository protocol`만 알아야 해요.
- hosted mode repository는 Flutter 모듈 내부 `network/auth/core`를 통해 API를 호출해요.
- 네이티브는 `hostContractVersion`, `featureId`, `SessionSnapshot`만 맞춰서 전달해요.
...
지금 v1에서 먼저 맞춰야 하는 것은 `hostContractVersion`, `featureId`, `session.userUuid`, 이벤트 이름이에요.
이 네 가지가 다르면 같은 feature도 플랫폼마다 다르게 동작할 수 있어요.

Host Contract를 맞출 때는 아래 다섯 가지를 먼저 같이 봐요.

- `hostContractVersion`, `featureId`, `session.userUuid` 이름과 타입이 완전히 같은지
- `SessionSnapshot`이 갱신될 때 기존 화면이 어떻게 반응하는지 같은지
- 이벤트 payload shape가 같은지
- 이벤트 이름과 발생 시점이 같은지
...
v1 launch payload는 아래 세 필드만 유지하는 것을 권장해요.
필드가 더 필요해 보이면 먼저 Flutter feature 내부에서 해결할 수 있는지 보고, 정말 host contract여야 할 때만 추가해요.
...
feature의 첫 화면, 초기 탭, 기본 필터 같은 시작 상태도 Flutter feature 내부에서 직접 결정하고 Host Contract에는 넣지 않아요.
...
- hosted mode와 같은 `HostLaunchContext` 최소 shape를 써요.
...
| launch contract | `hostContractVersion + featureId + SessionSnapshot` | `hostContractVersion + featureId + SessionSnapshot` |
| feature 코드 | 공통 | 공통 |
| usecase | 공통 | 공통 |
| repository protocol | 공통 | 공통 |
...
- `hostContractVersion + featureId + SessionSnapshot`을 전달해요.
...
- hosted/demo에서 같은 최소 launch context로 진입 가능한지 확인
...
- access token, base URL, 인증 로직은 Flutter 내부 `network/auth/core`가 소유하고 host contract로 넘기지 않아요.
...
- 두 플랫폼이 같은 최소 host contract를 전달해요.
...
hosted mode에서는 feature의 repository 구현체가 Flutter 내부 `network/auth/core`를 통해 서버 API를 직접 호출해요.
...
flowchart TD
    subgraph HOSTS["App Hosts"]
        ...
        subgraph AOS["AOS App"]
            ...
            A1["Native Feature A"]
            A2["Native Feature B"]
            U2["사용자가 AOS에서 팝업 관리자 진입"]
            A3["FlutterFeatureHost"]
        end
    end

    I3 --> H1["iOS host가 HostLaunchContext 조립
    hostContractVersion / featureId / SessionSnapshot"]
    ...
    H1 --> C1["Host Contract v1 최소 shape 통일
    hostContractVersion / featureId / session.userUuid / event name"]
    H2 --> C1
    ...
    subgraph FLUTTER["Flutter Feature Platform"]
        ...
        F5["review.management (future)"]
        F6["campaign.editor (future)"]
    end
