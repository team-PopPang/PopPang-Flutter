...
- v1에서 권한 있는 관리자 API 실행은 네이티브가 소유한다.
...
| repository 구현체 | Native bridge 호출형 | Mock / preset 데이터형 |
| 권한 있는 관리자 API 실행 | Native 소유 | 기본적으로 수행하지 않음 |
...
Flutter는 자기 feature의 `repository protocol`만 알고, hosted mode에서는 그 구현체가 native bridge를 통해 네이티브 로직과 연결됩니다.
...
### Q. Flutter가 관리자 API를 직접 호출하지 않는 이유는 무엇인가요?

현재 v1에서는 관리자 권한 경계를 Flutter 쪽으로 넓히지 않기 위해서입니다.  
권한 있는 관리자 실행은 계속 네이티브가 소유합니다.
